...
function form_general($smarty, $local_templates_dir, $arrBackupOptions, $module_name)
{
    ...
    $smarty->assign("TODO_PHONEVOX_CUSTOMS", _tr('Select all in this section'));
    //$smarty->assign("TODO_{GRUPO}", _tr('Select all in this section')); // Botão de selecionar todos da Categoria // [BKPH] Relacionado à backup.tpl
    ...
    $smarty->assign("PHONEVOX_CUSTOMS", _tr('Phonevox'));
    //$smarty->assign("{GRUPO}", _tr('TITULO DO GRUPO NA PAGE DE BACKUP')); // [BKPH] Relacionado à backup.tpl
    
    /*****************/
    // Itens (e descrição) da tabela
    ...
    $smarty->assign("backup_phonevox_customs", $arrBackupOptions['phonevox_customs']); // [BKPH] Relacionado à backup.tpl
    //$smarty->assign("backup_{grupo}", $arrBackupOptions['{GRUPO}']); // [BKPH] Para adição de grupo novo
...
function Array_Options($disabled="")
{
    $arrBackupOptions = array(
       ...
       "otros_new"      =>  array(
        ...
        "phonevox_customs"      =>  array( // Array com "bkp-category => descrição web"
            "int_ixcsoft"       =>  array("desc"=>_tr('Integracao  IXCsoft')),
            "int_sgp"           =>  array("desc"=>_tr('Integracao  SGP')),
            "int_receitanet"    =>  array("desc"=>_tr('Integracao  Receitanet')),
            "int_altarede"      =>  array("desc"=>_tr('Integracao  Altarede')),
        ),
        // [BKPH] Definição de um novo grupo, ou ensinando como editar um existente
        // "GRUPO"      =>  array( 
        //     "CATEGORIA"      =>  array("desc"=>_tr('DESC CATEGORIA')),
        //     "CATEGORIA"      =>  array("desc"=>_tr('DESC CATEGORIA')),
        // ),
    );
...
function boxAlert($smarty, $local_templates_dir, $versionList_current, $versionList_torestore, $compare)
{
    ...
    $smarty->assign(array(
        ...
        'Phonevox_customs'  =>  _tr('Phonevox customs'),
        // 'QUALQUER_NOME'  => _tr('TITULO DA SEÇÃO NA PG DE BKP'), // [BKPH] Geralmente pra GRUPOS novos
...
function getValueofBackupOption($valueOp)
{
    $arrayOptions = array(
        ...
        //"GRUPO" => array("CATEGORIA_1", "CATEGORIA_2"),
    );
    $arrayResult = array();
